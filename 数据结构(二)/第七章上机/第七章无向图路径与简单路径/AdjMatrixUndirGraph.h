...
template <class ElemType>
bool AdjMatrixUndirGraph<ElemType>::JudgePath(int v1, int v2)                     //判断v1是否可达v2
{
...
void AdjMatrixUndirGraph<ElemType>::GetSimplePath(int v1, int v2, int s) {
	...
	for (int i = 0; i < vexNum; i++)
	{
		if (arcs[v1][i] == 1 && tag[i] == UNVISITED)
		{
			tag[i] = VISITED;

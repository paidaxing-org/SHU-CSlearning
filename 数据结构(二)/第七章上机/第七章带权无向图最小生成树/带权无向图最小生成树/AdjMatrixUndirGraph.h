...
extern bool flag = 0;                   //判断是否可达的标志
extern int re[2000] = { 0 };            //用于记录遍历当前位置经过的顶点下标
extern int s = 0;                       //遍历次数，同样用于给re数组定位
queue<int> q;
// 无向图的邻接矩阵类
...
public:
	...
	void JudgePath(int v1, int v2);
	void GetSimplePath(int v1, int v2, int s);
	void getMinTree_Prim(int start);
...
template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::JudgePath(int v1, int v2)                     //判断v1是否可达v2
{
	...
	if (v1 >= vexNum || v1 < 0 || v2 >= vexNum || v2 < 0)
		throw("输入的顶点不合法！");
	q.push(v1);
	tag[v1] = VISITED;
	while (!q.empty())
...
//使用BFS对图从给出的v1点开始遍历，每次将遍历到的点的邻接矩阵的中数值为1的边对应的终点作为新的起点

template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::GetSimplePath(int v1, int v2, int s) {
	if (s == 0)
	{
		for (int i = 0; i < vexNum; i++)
			tag[i] = UNVISITED;
	...
	for (int i = 0; i < vexNum; i++)
	{
		if (arcs[v1][i] != 0 && tag[i] == UNVISITED)
		{
			tag[i] = VISITED;
			GetSimplePath(i, v2, s + 1);
			tag[i] = UNVISITED;
		}
	}
...
template <class ElemType>
void AdjMatrixUndirGraph<ElemType>::getMinTree_Prim(int start)
{
	int minnum;
	int vexnum = this->GetVexNum();
	CloseArc<ElemType>* closearc;
	if (start < 0 || start >= vexnum)
	{
		cout << "顶点" << start << "不存在" << endl;
		return;
	}
	int u, v, k;
	closearc = new CloseArc<ElemType>[vexnum];
	for (v = 0; v < vexnum; v++)
	{
		if (v != start)
		{
			closearc[v].nearvertex = start;
			closearc[v].lowweight = this->getWeight(start, v);
		}
	}
	closearc[start].nearvertex = -1;
	closearc[start].lowweight = 0;
	int op1, op2;
	for (k = 1; k < vexnum; k++)
	{
		minnum = 1e9;
		...
		for (u = 0; u < vexnum; u++)
		{
			if (closearc[u].lowweight != 0 && closearc[u].lowweight < minnum)
			{
				v = u;
				minnum = closearc[u].lowweight;
			}
		}
		if (v != start)
		{
			op1 = closearc[v].nearvertex;
			op2 = v;
			cout << "边(" << op1 << "," << op2 << ")的权值为:" << minnum << endl;
			closearc[v].lowweight = 0;
			for (u = FirstAdjVex(v); u != -1; u = NextAdjVex(v, u))
			{
				if (closearc[u].lowweight != 0 && getWeight(v, u) < closearc[u].lowweight)
				{
